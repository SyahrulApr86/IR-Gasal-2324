...
def sort_intersect_multiple_lists(*lists):
    """
    Intersects multiple (ascending) sorted lists and returns the sorted result
    Melakukan Intersection beberapa (ascending) sorted lists dan mengembalikan hasilnya
    yang juga terurut.
    """
    if len(lists) == 1:
        return lists[0]
    if len(lists) == 2:
        return sort_intersect_list(lists[0], lists[1])

    mid = len(lists) // 2
    left = sort_intersect_multiple_lists(*lists[:mid])
    right = sort_intersect_multiple_lists(*lists[mid:])

    return sort_intersect_list(left, right)
