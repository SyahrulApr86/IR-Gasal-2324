...
class InvertedIndexReader(InvertedIndex):
    ...
    def get_postings_list(self, term):
        ...
        # seek ke posisi byte di index file untuk membaca postings list dan tf list
        self.index_file.seek(pos)
        postings_list_encoded = self.index_file.read(len_in_bytes_of_postings)
        tf_list_encoded = self.index_file.read(len_in_bytes_of_tf)
...
class InvertedIndexWriter(InvertedIndex):
    ...
    def append(self, term, postings_list, tf_list):
        ...
        len_in_bytes_of_postings = len(encoded_postings)
        len_in_bytes_of_tf = len(encoded_tf_list)

        self.terms.append(term)
        self.postings_dict[term] = (start_position, len(postings_list), len_in_bytes_of_postings, len_in_bytes_of_tf)

        # Update doc_length
        for docID, tf in zip(postings_list, tf_list):
            if docID in self.doc_length:
                self.doc_length[docID] += tf
            else:
                self.doc_length[docID] = tf

        # 4. Menambahkan (append) bystream ke posisi akhir index file di harddisk.
