...
from letor import rerank_search_results, train_letor, rerank_with_letor
...
def eval_retrieval(qrels, query_file="./qrels-folder/test_queries.txt", k=1000):
    ...
    for k1 in k1_values:
        ...
        # Evaluasi untuk LETOR
        rbp_scores_letor = []
        dcg_scores_letor = []
        ap_scores_letor = []

        ranker, lsi_model, dictionary = train_letor()
        ...
        for qline in tqdm(queries):
            ...
            # Dapatkan SERP dari BSBI (TF-IDF atau BM25)
            SERP = BSBI_instance.retrieve_tfidf(search_query, k=k)  # contoh menggunakan TF-IDF

            # Rerank dengan LETOR
            reranked_SERP = rerank_with_letor(ranker, lsi_model, dictionary, SERP, search_query)

            # Buat ranking untuk evaluasi
            ranking_letor = []
            for doc_id, _ in reranked_SERP:
                did = int(doc_id.split('.')[0])
                if (did in qrels[qid]):
